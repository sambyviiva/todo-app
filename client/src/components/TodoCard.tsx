...
export const TodoCard = ({ todo, updateTodoFn, removeTodoFn }: Props) => {
    ...
    return (
        <div className="rounded border-black border-4 p-4 mb-4 bg-indigo-950 w-100 relative">
            <button className="absolute top-4 right-4 p-1 text-sm italic" onClick={() => removeTodoFn(todo.id)}>🗑</button>
            <div className="text-2xl text-center mb-1 font-bold">{todo.title}</div>
            <div className="h-30">{todo.description}</div>
